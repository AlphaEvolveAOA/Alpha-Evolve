...
class GameState:
    ...
    GameStates are used by the Game obj to capture the actual state of the game and
    can be used by agents to reason about the game.

    Much of the information in a GameState is stored in a GameStateData obj.  We
    strongly suggest that you access that data via the accessor methods below rather
    than referring to the GameStateData obj directly.
...
#        GameState.explored.add(self)
    ...
    def get_pacman_state(self):
        """
        Returns an AgentState obj for pacman (in game.py)
...
def default(str_param):
    return str_param + ' [Default: %default]'

def parse_agent_args(str_param):
    if str_param is None: return {}
    pieces = str_param.split(',')
    opts = {}
    for p in pieces:
        if '=' in p:
            key, val = p.split('=')
