...
class LLMGuidedMutator(BaseMutator):
    ...
    def generate(self, parents: list[Candidate], generation: int,
        ...
        for i in range(crossover_count):
            ...
            try:
                ...
                change_summary = LLMGuidedMutator._describe_code_change(parent_a.code, new_code)
                desc = f"LLM crossover {parent_a.code_hash[:8]}+{parent_b.code_hash[:8]} | {change_summary}"
            except Exception as e:
        ...
        for i in range(mutation_count):
            ...
            try:
                ...
                change_summary = LLMGuidedMutator._describe_code_change(parent.code, new_code)
                desc = f"LLM-guided (temp={temperature:.2f}, {len(rag_examples)} RAG) | {change_summary}"
            except Exception as e:
    ...
    @staticmethod                         
    def _describe_code_change(old_code: str, new_code: str) -> str:
        ...
        parts = []
        if removed:
            parts.append(f"removed: {removed[0][:60]}")
        if added:
            parts.append(f"added: {added[0][:60]}")
