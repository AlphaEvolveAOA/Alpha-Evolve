...
This takes a while the first time because it pulls in PyTorch and the sentence-transformers library. Might feel like an Eternity.
...
The results varies as the inputs such as weight, population size, generation size changes

After running, check the below files for analysis:
...
- The **chart PNG** for visualizations 
